...
test.describe('mobile baseline', () => {
  test('TomorrowStar -> Diagnosis keeps 30-day mobile path', async ({ page, mockApp, isMobile }) => {
    test.skip(!isMobile, 'mobile baseline only validates phone/tablet projects')
  ...
  test('Watchlist and Config render mobile-specific entry points', async ({ page, mockApp, isMobile }) => {
    test.skip(!isMobile, 'mobile baseline only validates phone/tablet projects')

    await mockApp({ admin: true })
    await page.goto('/watchlist')
    await expect(page.getByText('我的观察')).toBeVisible()
    await expect(page.getByRole('button', { name: /000001/ })).toBeVisible()
    await expect(page.getByText('建仓建议')).toBeVisible()
