...
export default defineConfig(({ mode }) => {
  ...
  const backendPort = parseInt(env.BACKEND_PORT || '8000')
  const backendHost = env.BACKEND_HOST || '127.0.0.1'
  const normalizedBackendHost = backendHost === '0.0.0.0' ? 'localhost' : backendHost
  ...
  return {
    ...
    server: {
      ...
      proxy: {
        ...
        '/ws': {
          target: `ws://${normalizedBackendHost}:${backendPort}`,
          ws: true
        }
      }
    }
