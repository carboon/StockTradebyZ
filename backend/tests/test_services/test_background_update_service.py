...
def _seed_stock_daily(
    test_db,
    *,
    code: str,
    trade_date: str,
    turnover_rate: float | None = 1.2,
...
def test_assess_freshness_detects_missing_volume_ratio_on_latest_trade_date(test_db, tmp_path, monkeypatch) -> None:
    import app.services.background_update_service as background_update_module
    ...
    assert status.latest_trade_date == "2026-05-08"
    assert status.latest_db_date == "2026-05-08"
    assert status.db_needs_update is True
    assert status.needs_market_update is True
    assert "量比不完整" in status.reason
